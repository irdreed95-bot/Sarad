...
export default function WatchPage() {
  ...
  // Dynamic servers from admin store
  ...
  const activateServer = (serverId: string) => {
    ...
    if (contentType === "movie") {
      ...
      setPlayerActive(false);
      setTimeout(() => setPlayerActive(true), 60);
    }
  };
    ...
    <div className="min-h-screen bg-black text-white pb-28 md:pb-10">
      ...
      <div className="w-full bg-zinc-950 shadow-xl">
        <div className="relative w-full" style={{ paddingTop: "56.25%" }}>
          {showPlayer ? (
            <iframe
              key={`${tmdbId}-${activeServerId}-${selectedSeason}-${selectedEpisode}`}
              src={getIframeSrc()}
            ...
            <>
              {backdrop && <img src={backdrop} alt={title} className="absolute inset-0 w-full h-full object-cover opacity-30" />}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className={`flex flex-wrap items-center gap-2 mb-3 ${isRTL ? "flex-row-reverse" : ""}`}>
                  {data.vote_average && data.vote_average > 0 ? (
                    <span className="flex items-center gap-1 text-primary font-bold text-sm"><Star size={13} fill="currentColor" />{data.vote_average.toFixed(1)}</span>
                  ) : null}
                  ...
                  <span className={`text-[11px] px-2.5 py-0.5 rounded-full font-bold border ${
                    contentType === "tv" ? "bg-blue-500/15 text-blue-400 border-blue-500/30" : "bg-primary/15 text-primary border-primary/30"
                  }`}>
                ...
                {data.overview && <p className="text-white/65 text-sm leading-relaxed max-w-2xl line-clamp-4">{data.overview}</p>}
              </div>

              <button onClick={toggleBookmark} className="flex-shrink-0 flex flex-col items-center gap-1 p-3 rounded-xl bg-zinc-900 hover:bg-zinc-800 transition-colors border border-white/8">
                {bookmarked ? <BookmarkCheck size={20} className="text-primary" /> : <Bookmark size={20} className="text-white/50" />}
                ...
                {seasonDropdownOpen && (
                  <div className="absolute right-0 top-full mt-1 z-30 bg-zinc-900 border border-white/10 rounded-xl shadow-2xl overflow-hidden min-w-[180px]">
                    {validSeasons.map(s => (
                      <button
                        key={s.season_number}
                        onClick={() => { setSelectedSeason(s.season_number); setSeasonDropdownOpen(false); setPlayerActive(false); }}
                        className={`w-full text-left px-4 py-3 text-sm transition-colors hover:bg-zinc-800 ${selectedSeason === s.season_number ? "text-primary font-semibold bg-primary/10" : "text-white/80"}`}
                      >
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-start gap-3 p-3 rounded-2xl border text-left transition-all duration-200 group/ep hover:scale-[1.01] ${
                        isPlaying ? "bg-primary/10 border-primary/50 shadow-[0_0_20px_rgba(212,175,55,0.2)]" : "bg-zinc-900/70 border-white/6 hover:border-primary/30 hover:bg-zinc-900"
                      }`}
                    >
                      <div className="relative flex-shrink-0 w-28 rounded-xl overflow-hidden bg-zinc-800" style={{ aspectRatio: "16/9" }}>
                        {still ? <img src={still} alt={ep.name} className="w-full h-full object-cover" loading="lazy" /> : <div className="w-full h-full flex items-center justify-center"><Tv size={22} className="text-white/20" /></div>}
                      ...
                      <div className="flex-1 min-w-0 pt-0.5">
                        <div className={`flex items-center gap-2 mb-1 ${isRTL ? "flex-row-reverse" : ""}`}>
                          <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${isPlaying ? "bg-primary text-black" : "bg-zinc-800 text-white/50"}`}>
                            {t("ح", "E")}{ep.episode_number}
                          </span>
                          {isPlaying && <span className="text-primary text-[11px] font-semibold animate-pulse">{t("يعرض الآن ●", "Now Playing ●")}</span>}
                        ...
                        {ep.vote_average && ep.vote_average > 0 ? (
                          <p className="flex items-center gap-1 text-primary text-[10px] mt-1.5"><Star size={8} fill="currentColor" />{ep.vote_average.toFixed(1)}</p>
                        ) : null}
                      </div>
                    </button>
                  );
